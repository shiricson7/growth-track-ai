...
export default function IntakesDashboard({ embedded = false }: { embedded?: boolean }) {
  const containerClass = embedded ? "w-full" : "min-h-screen bg-slate-50 p-4 md:p-8";
  const innerClass = "max-w-6xl mx-auto space-y-6";
  const [filter, setFilter] = useState<FilterId>('today');
  const [rows, setRows] = useState<IntakeRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [hasSession, setHasSession] = useState(false);
  const [role, setRole] = useState<'owner' | 'staff' | 'tablet' | 'member' | null>(null);
  ...
  return (
    <div className={containerClass}>
      <div className={innerClass}>
        ...
        {hasSession && accessDenied && (
          <div className="bg-white border border-slate-200 rounded-xl p-6 text-center text-slate-500">
          권한이 없습니다.
          </div>
        )}

        {hasSession && !accessDenied && loading && (
          <div className="bg-white border border-slate-200 rounded-xl p-6 text-center text-slate-500">로딩 중...</div>
        )}

        {hasSession && !accessDenied && !loading && error && (
          <div className="bg-red-50 border border-red-100 rounded-xl p-6 text-red-600">{error}</div>
        )}

        {hasSession && !accessDenied && !loading && !error && rows.length === 0 && (
          <div className="bg-white border border-slate-200 rounded-xl p-6 text-center text-slate-500">
            표시할 문진이 없습니다.
          </div>
        )}

        {hasSession && !accessDenied && !loading && rows.length > 0 && (
