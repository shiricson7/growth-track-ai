...
export const weightStandards = {
  async load() {
    if (isLoaded) return;
    try {
      const response = await fetch('/weight_chart_LMS.csv');
      ...
      console.error('Failed to load weight standards:', error);
    }
  },

  calculatePercentile(gender: 'Male' | 'Female' | 'male' | 'female', ageYears: number, weight: number): number | null {
    if (!isLoaded || !weight) return null;
